### [M-1] Looping through players array to check for duplicates in `PuupyRaffle::enterRaffle` is a potential denial of service (DOS) attack, incrementing gas costs for future entrants 
...
If we have 2 sets of 1000 players enter, the gas cost will be as such:
- Gas cost of the first 1000 players 417,422,122
- Gas cost of the second 1000 players 1,600,813,067

This is 3.8x more expensive for the second number of players.

<details>
<summary>PoC</summary>
Place the following test info `PuppyRaffleTest.t.sol`.
...
```Javascript
    function testDOSEnterRaffle() public {
        ...
        uint256 gasUsedFirstOneThousandPlayers = (gasStart - gasEnd) * tx.gasprice;

        console.log("Gas cost of the first 1000 players", gasUsedFirstOneThousandPlayers);

        //now 2nd 1000 players
        address[] memory playersTwo = new address[](playersNum);
        for (uint256 i = 0; i < playersNum; i++) {
            playersTwo[i] = address(i + playersNum); //This will continue from 1000 -> 1001, 1002, 1003, ...
        }
        uint256 gasStartSecond = gasleft();
        puppyRaffle.enterRaffle{value: entranceFee * players.length}(playersTwo);
        uint256 gasEndSecond = gasleft();

        uint256 gasUsedSecondOneThousandPlayers = (gasStartSecond - gasEndSecond) * tx.gasprice;

        console.log("Gas cost of the second 1000 players", gasUsedSecondOneThousandPlayers);

        assert(gasUsedFirstOneThousandPlayers < gasUsedSecondOneThousandPlayers);
    }
```
</details>
...
1. Consider allowing duplicates. Users can make new wallet addresses anyways, so duplicate checks doesn't prevent the same person from entering multiple times, only the same wallet address.
2. Consider using a mapping to check for duplicates. This would allow const time lookup of whether a user has already entered.
